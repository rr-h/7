...
<!-- We have already mentioned <i>event handlers</i> that are registered to be called when specific events occur a few times in [cardiology](/en/cardiology). E.g. a user's interaction with the different elements of a web page can cause a collection of various different kinds of events to be triggered.-->
...
<!-- This means we can simplify the component by using [destructuring](/en/dermatology/component_state_event_handlers#destructuring), like so:-->
