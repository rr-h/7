...
Nous avons déjà mentionné les <i>gestionnaires d'événements</i> qui sont enregistrés pour être appelés lorsque des événements spécifiques se produisent plusieurs fois dans la [partie 0](/fr/cardiology). Par exemple. l'interaction d'un utilisateur avec les différents éléments d'une page Web peut provoquer le déclenchement d'un ensemble de différents types d'événements.
...
Cela signifie que nous pouvons simplifier le composant en utilisant [la destructuration](/fr/dermatology/etat_des_composants_gestionnaires_devenements#destructuration), comme ceci :
