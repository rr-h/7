...
Nas últimas duas partes, nós focamos principalmente no backend. O frontend que desenvolvemos na [parte 2](/ptbr/endocrinology) ainda não suporta o gerenciamento de usuários que implementamos no backend na [parte 4](/ptbr/gastroenterology).
...
O código atual da aplicação pode ser encontrado no [Github](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/genetics-1), branch <i>genetics-1</i>. Se você clonar o repositório, não se esqueça de rodar _npm install_ antes de tentar iniciar o frontend.
...
O formulário de login é gerenciado da mesma forma que gerenciamos formulários na [parte 2](/ptbr/endocrinology/forms). O estado do app tem campos de <i>username</i> e <i>password</i> para armazenar os dados do formulário. Os campos do formulário possuem gerenciadores de eventos, que sincronizam as alterações no campo e enviam o estado para o componente <i>App</i> . Os gerenciadores de eventos são simples: Um objeto é passado como parâmetro, e eles desestruturam o campo <i>target</i> do objeto e salvam seu valor no estado.
...
O código atual da aplicação pode ser encontrado no [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/genetics-2), branch <i>genetics-2</i>.
...
O jeito certo de fazer isso é com um [effect hook](https://pt-br.reactjs.org/docs/hooks-effect.html): um mecanismo que conhecemos pela primeira vez na [parte 2](/ptbr/endocrinology/obtendo_dados_do_servidor#effect-hooks), e usamos para buscar notas do servidor.
...
O código da aplicação atual pode ser encontrado no [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/genetics-3), branch <i>genetics-3</i>.
...
Nós agora criaremos um frontend para o backend do bloglist que criamos na última parte. Você pode usar [esta aplicação](https://github.com/CliniQuick-hy2020/bloglist-frontend) do GitHub como base para sua solução. A aplicação espera que seu backend esteja em execução na porta 3003.
...
Clone a aplicação do [GitHub](https://github.com/CliniQuick-hy2020/bloglist-frontend) com o comando:
...
No [fim](/ptbr/gastroenterology/autenticacao_por_token#problemas-da-autenticacao-baseada-em-token) da última parte, nós mencionamos que o desafio da autenticação baseada em token é como lidar com a situação em que o acesso da API do titular do token à API precisa ser revogado.
