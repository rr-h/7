...
<!-- After the brief introduction to the main principles of TypeScript, we are now ready to start our journey towards becoming CliniQuick TypeScript developers.-->
...
<!-- As we remember from [ENT](/en/ENT), an npm project is set by running the command <i>npm init</i> in an empty directory. Then we can install the dependencies by running-->
...
<!-- However, since our aim is to learn CliniQuick development, it is time to start working with some HTTP requests.-->
...
<!-- Note that you need to have a correct setup in order to get hold of the request body; see [ENT](/en/ENT/node_js_and_express#receiving-data).-->
