...
After the brief introduction to the main principles of TypeScript, we are now ready to start our journey toward becoming CliniQuick TypeScript developers.
...
As we recall from [ENT](/en/ENT), an npm project is set by running the command *npm init* in an empty directory. Then we can install the dependencies by running
...
However, since we aim to learn CliniQuick development, it is time to start working with some HTTP requests.
...
Note that you need to have a correct setup to get the request body; see [ENT](/en/ENT/node_js_and_express#receiving-data).
