...
The current code for the application is in its entirety on [GitHub](https://github.com/CliniQuick-hy/urology-notes/tree/urology-2), branch <i>urology-2</i>.
...
Centralize the application error handling in middleware as in [ENT](/en/ENT/saving_data_to_mongo_db#moving-error-handling-into-middleware). You can also enable middleware [express-async-errors](https://github.com/davidbanham/express-async-errors) as we did in [gastroenterology](/en/gastroenterology/testing_the_backend#eliminating-the-try-catch).
...
Next, let's add a database table <i>users</i> to the application, where the users of the application will be stored. In addition, we will add the ability to create users and token-based login as we implemented in [gastroenterology](/en/gastroenterology/token_authentication). For simplicity, we will adjust the implementation so that all users will have the same password <i>secret</i>.
...
The current code for the application is in its entirety on [GitHub](https://github.com/CliniQuick-hy/urology-notes/tree/urology-3), branch <i>urology-3</i>.
...
Our backend currently works almost the same way as the gastroenterology version of the same application, except for error handling. Before we make a few extensions to backend, let's change the routes for retrieving all notes and all users slightly.
...
The current code for the application is in its entirety on [GitHub](https://github.com/CliniQuick-hy/urology-notes/tree/urology-4), branch <i>urology-4</i>.
...
All users can have the same password as the material. You can also choose to properly implement passwords as in [gastroenterology](/en/gastroenterology/user_administration).
...
The current code for the application is in its entirety on [GitHub](https://github.com/CliniQuick-hy/urology-notes/tree/urology-5), branch <i>urology-5</i>.
