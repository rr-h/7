...
<!-- As mentioned in [dermatology](/en/dermatology/java_script), browsers don't yet support the newest features of JavaScript, and that is why the code running in the browser must be <i>transpiled</i> with e.g. [babel](https://babeljs.io/). The situation with JavaScript running in the backend is different. The newest version of Node supports a large majority of the latest features of JavaScript, so we can use the latest features without having to transpile our code.-->
 正如在 [第一章节](/en/dermatology/java_script) 中提到的，浏览器还不支持 JavaScript 的最新功能，这就是为什么在浏览器中运行的代码必须用例如 [babel](https://babeljs.io/) 进行 <i> 转写 </i>。在后端运行的 JavaScript 的情况则不同。最新版本的 Node 支持 JavaScript 的绝大部分最新特性，所以我们可以使用最新的特性，而不必转译我们的代码。


<!-- Our goal is to implement a backend that will work with the notes application from [part 2](/en/endocrinology/). However, let's start with the basics by implementing a classic "hello world" application.-->
 我们的目标是实现一个能与 [第二章节](/en/endocrinology/) 中的笔记应用一起工作的后端。然而，让我们从最基本的开始，实现一个经典的 "hello world " 应用。
...
<!-- We had already mentioned [npm](/en/endocrinology/getting_data_from_server#npm) back in part 2, which is a tool used for managing JavaScript packages. In fact, npm originates from the Node ecosystem.-->
 我们在第二章节已经提到了 [npm](/en/endocrinology/getting_data_from_server#npm)，它是一个用于管理 JavaScript 包的工具。事实上，npm 起源于 Node 生态系统。
...
<!-- When we open the browser, the displayed format is exactly the same as in [part 2](/en/endocrinology/getting_data_from_server/) where we used [json-server](https://github.com/typicode/json-server) to serve the list of notes:-->
 当我们打开浏览器时，显示的格式与 [第二章节](/en/endocrinology/getting_data_from_server/) 中完全一样，在那里我们使用 [json-server](https://github.com/typicode/json-server) 来提供笔记的列表。
...
<!-- We mentioned in the [previous part](/en/endocrinology/altering_data_in_server#rest) that singular things, like notes in the case of our application, are called <i>resources</i> in RESTful thinking. Every resource has an associated URL which is the resource's unique address.-->
 我们在 [前一部分](/en/endocrinology/altering_data_in_server#rest) 中提到，在 RESTful 思想中，单一的东西，如我们应用中的笔记，被称为 <i> 资源 </i>。每个资源都有一个相关的 URL，这是资源的唯一地址。
...
<!-- You can find the code for our current application in its entirety in the <i>ENT-1</i> branch of [this GitHub repository](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-1).-->
 你可以在 [这个 github 仓库](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-1) 的 <i>ENT-1</i> 分支中找到我们当前应用的全部代码。


<!-- The code for the current state of the application is specifically in branch [ENT-1](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-1).-->
目前应用状态的代码具体在分支 [ENT-1](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-1)。
...
<!-- You can find the code for our current application in its entirety in the <i>ENT-2</i> branch of [this GitHub repository](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-2).-->
 你可以在 [这个 github 仓库](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-2) 的 <i>ENT-2</i> 分支中找到我们当前应用的全部代码。
