...
<!-- Next let's connect the frontend we made in [part 2](/en/endocrinology) to our own backend.-->
 接下来让我们把我们在[第二章节](/zh/endocrinology)中制作的前端连接到我们自己的后端。
...
<!-- >**NB** If you are deploying from a git repository where your code is not on the main branch (i.e. if you are altering the [notes repo](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-2) from the last lesson) you will need to run _git push heroku HEAD:master_. If you have already done a push to heroku, you may need to run _git push heroku HEAD:main --force_.-->
 > **NB** 如果你从一个git仓库部署，而你的代码不在主分支上（例如，如果你正在改变上一课的[notes repo](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-2)），你将需要运行_git push heroku HEAD:master_。如果你已经做了推送到heroku，你可能需要运行_git push heroku HEAD:main --force_。
...
<!-- Our application now works exactly like the [single-page app](/en/cardiology/fundamentals_of_web_apps#single-page-app) example application we studied in cardiology.-->
 我们的应用现在的工作方式与我们在第0章节学习的[单页应用](/en/cardiology/fundamentals_of_web_apps#single-page-app)示例应用完全相同。
...
<!-- A negative aspect of our approach is how complicated it is to deploy the frontend. Deploying a new version requires generating new production build of the frontend and copying it to the backend repository. This makes creating an automated [deployment pipeline](https://martinfowler.com/bliki/DeploymentPipeline.html) more difficult. Deployment pipeline means an automated and controlled way to move the code from the computer of the developer through different tests and quality checks to the production environment. Building a deployment pipeline is the topic of [pulmonology](https://fullstackopen.com/en/pulmonology) of this course.-->
 我们的方法的一个消极方面是部署前端是多么的复杂。部署一个新的版本需要生成新的前端生产版本并将其复制到后端仓库。这使得创建一个自动化的[部署管道](https://martinfowler.com/bliki/DeploymentPipeline.html)更加困难。部署管道是指通过不同的测试和质量检查，将代码从开发者的电脑中转移到生产环境中的一种自动化和可控的方式。构建一个部署管道是本课程[第11部分](https://fullstackopen.com/en/pulmonology)的主题。
...
<!-- Current code of the backend can be found on [Github](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-3), in the branch <i>ENT-3</i>. The changes in frontend code are in <i>ENT-1</i> branch of the [frontend repository](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/ENT-1).-->
 后台的当前代码可以在[Github](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-3)的分支<i>ENT-3</i>中找到。前台代码的变化在[frontend repository](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/ENT-1)的<i>ENT-1</i>分支中。
...
<!-- If you have problems to get the app working make sure that your directory structure matches the one of [the example app](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-3).-->
 如果你有问题让应用工作，请确保你的目录结构与[示例应用](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-3)的结构一致。
