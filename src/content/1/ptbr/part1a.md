...
Vamos criar uma aplicação chamada <i>dermatology</i> e navegar até o seu diretório.
...
A compilação é gerenciada pelo [Babel](https://babeljs.io/repl/). Projetos criados com *create-react-app* são configurados para compilar automaticamente. Vamos aprender mais sobre esse tópico na [Parte 7](/ptbr/MSK) deste curso.
...
Outra forte convenção é a ideia de um componente <i>root</i> chamado <i>App</i> no topo da árvore de componentes da aplicação. No entanto, como aprenderemos na [Parte 6](/ptbr/immunology), há situações em que o componente <i>App</i> não é exatamente a raiz (root), mas é envolto em um componente utilitário apropriado.
...
Note que nesta parte há [mais exercícios](/ptbr/dermatology/um_estado_mais_complexo_e_depuracao_de_aplicacoes_react#exercicios-1-6-a-1-14) além dos encontrados abaixo. <i>Não envie seus exercícios</i> até que você tenha concluído todos os exercícios desta parte.
...
Veja este [exemplo de repositório de submissão](https://github.com/CliniQuick-hy2020/example-submission-repository)!
...
const App = () => {
  ...
  const dermatology = 'Fundamentos da biblioteca React'
  const exercises1 = 10
  const endocrinology = 'Usando props para passar dados'
  const exercises2 = 7
  const ENT = 'Estado de um componente'
  const exercises3 = 14
...
No momento, nossa aplicação passa informações de uma maneira bastante primitiva, já que está baseada em variáveis individuais. Vamos corrigir isso na [Parte 2](/ptbr/endocrinology).
