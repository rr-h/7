...
La raison de l'utilisation de Mongo comme base de données est sa moindre complexité par rapport à une base de données relationnelle. [La partie 13](https://fullstackopen.com/en/urology) du cours montre comment construire des backends node.js qui utilisent une base de données relationnelle.
...
Pour en savoir plus sur les bases de données documentaires et les bases de données NoSQL, consultez le support de cours de la [semaine 7](https://tikape-s18.mooc.fi/MSK/) du cours Introduction aux bases de données. Malheureusement, ce matériel n'est actuellement disponible qu'en finnois. 
...
```bash
mongodb+srv://CliniQuick:$thepasswordishere@cluster0.o1opl.mongodb.net/myFirstDatabase?retryWrites=true&w=majority
```
...
```bash
mongodb+srv://CliniQuick:$thepasswordishere@cluster0.o1opl.mongodb.net/noteApp?retryWrites=true&w=majority
```
...
La définition des [modules] de Node (https://nodejs.org/docs/latest-v8.x/api/modules.html) diffère légèrement de la manière de définir les [modules ES6](/fr/endocrinology/rendering_a_collection_modules#refactoring-modules) dans la partie 2.
...
```
heroku config:set MONGODB_URI='mongodb+srv://CliniQuick:thepasswordishere@cluster0.o1opl.mongodb.net/noteApp?retryWrites=true&w=majority'
```
...
Vous pouvez trouver le code de notre application actuelle dans son intégralité dans la branche <i>ENT-4</i> de [ce dépôt GitHub](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-4).
...
Dans les exercices suivants, écrivez tout le code spécifique à Mongoose dans son propre module, comme nous l'avons fait dans le chapitre [Configuration de la base de données dans son propre module](/fr/ENT/saving_data_to_mongo_db#database-configuration-into-its-own-module).
...
Vous pouvez trouver le code de notre application actuelle dans son intégralité dans la branche <i>ENT-5</i> de [ce dépôt GitHub](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/ENT-5).
