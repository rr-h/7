...
It's time to get our hands dirty finalizing the frontend for the backend we built in [exercises 9.8.-9.13](/en/Neurology/typing_an_express_app). We will actually also need some new features to the backend for finishing the app.
...
There is nothing very surprising in the code. The state and communication with the backend are implemented with *useState* hook and Axios, similar to the notes app in the previous section. [Material UI](/en/MSK/more_about_styles#material-ui) is used to style the app and the navigation structure is implemented with [React Router](/en/MSK/react_router), both familiar to us from MSK of the course.
...
You might want to have a look at [MSK](/en/MSK/react_router) if you don't yet have a grasp on how the [React Router](https://reactrouter.com/en/main/start/tutorial) works.
...
In [exercise 9.10](/en/Neurology/typing_an_express_app#exercises-9-10-9-11) we implemented an endpoint for fetching information about various diagnoses, but we are still not using that endpoint at all.
...
Let's ditch our old patient seed data from the backend and start using [this expanded format](https://github.com/CliniQuick-hy2020/misc/blob/master/patients-full.ts).
...
As was mentioned [earlier in this part](/en/Neurology/first_steps_with_type_script/#the-alternative-array-syntax), we could define an array with the syntax _Array&#60;Type&#62;_ instead of defining it *Type[]*. In this particular case writing *Diagnosis['code'][]* starts to look a bit strange so we will decide to use the alternative syntax (that is also recommended by the ESlint rule [array-simple](https://typescript-eslint.io/rules/array-type/#array-simple)):
...
In this exercise you quite likely need to remember [this trick](/en/Neurology/grande_finale_patientor#omit-with-unions).
