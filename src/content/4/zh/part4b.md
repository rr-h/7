...
<!-- You can find the code for our current application in its entirety in the <i>gastroenterology-2</i> branch of [this GitHub repository](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-2).-->
 你可以在[这个github仓库](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-2)的<i>gastroenterology-2</i>分支中找到我们当前应用的全部代码。
...
<!-- You can find the code for our current application in its entirety in the <i>gastroenterology-3</i> branch of [this GitHub repository](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-3).-->
 你可以在[这个Github仓库](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-3)的<i>gastroenterology-3</i>分支中找到我们当前应用的全部代码。
...
<!-- You can find the code for our current application in its entirety in the <i>gastroenterology-4</i> branch of [this GitHub repository](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-4).-->
 你可以在[这个Github仓库](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-4)的<i>gastroenterology-4</i>分支中找到我们当前应用的全部代码。
...
<!-- The code for our application can be found from [github](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-5), branch <i>gastroenterology-5</i>.-->
 我们应用的代码可以从[github](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-5)，分支<i>gastroenterology-5</i>找到。
...
<!-- Notice that you will have to make similar changes to the code that were made [in the material](/en/gastroenterology/testing_the_backend#test-environment), like defining the test environment so that you can write tests that use their own separate database.-->
 注意，你将不得不对[材料中](/en/gastroenterology/testing_the_backend#test-environment)的代码进行类似的修改，比如定义测试环境，以便你可以编写使用自己独立数据库的测试。
...
<!-- **NB:** when you are writing your tests **<i>it is better to not execute all of your tests</i>**, only execute the ones you are working on. Read more about this [here](/en/gastroenterology/testing_the_backend#running-tests-one-by-one).-->
 **NB:**当你写你的测试时，**<i>最好不要执行你所有的测试</i>**，只执行你正在进行的测试。阅读更多关于这个的信息[这里](/en/gastroenterology/testing_the_backend#running-tests one-by-one)。
...
<!-- Make the required changes to the code so that it passes the test. The [toJSON](/en/ENT/saving_data_to_mongo_db#backend-connected-to-a-database) method discussed in ENT is an appropriate place for defining the <i>id</i> parameter.-->
 对代码进行必要的修改，使其通过测试。第3章节中讨论的[toJSON](/en/ENT/saving_data_to_mongo_db#backend-connected-to-a-database)方法是定义<i>id</i>参数的一个合适位置。
...
<!-- You can find the code for our current application in its entirety in the <i>gastroenterology-6</i> branch of [this GitHub repository](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-6).-->
 你可以在[这个Github仓库](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-6)的<i>gastroenterology-6</i>分支中找到我们当前应用的全部代码。
...
<!-- Use the async/await syntax. Follow [RESTful](/en/ENT/node_js_and_express#rest) conventions when defining the HTTP API.-->
 使用async/await语法。在定义HTTP API时遵循[RESTful](/en/ENT/node_js_and_express#rest)惯例。
...
<!-- The application mostly needs to update the amount of <i>likes</i> for a blog post. You can implement this functionality the same way that we implemented updating notes in [ENT](/en/ENT/saving_data_to_mongo_db#other-operations).-->
 应用主要需要更新一篇博客文章的<i>喜欢</i>的数量。你可以用我们在[第三章节](/en/ENT/saving_data_to_mongo_db#other-operations)中实现更新笔记的方法来实现这一功能。
