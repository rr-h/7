...
<!--     - We will add the login form to the frontend in [genetics](/en/genetics)-->
 - 我们将在[第五章节](/en/genetics)中把登录表单添加到前端。
...
<!-- Current application code can be found on [Github](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-9), branch <i>gastroenterology-9</i>.-->
 目前的应用代码可以在[Github](https://github.com/CliniQuick-hy2020/ENT-notes-backend/tree/gastroenterology-9)上找到，分支<i>gastroenterology-9</i>。
...
<!-- We will implement login to the frontend in the [next part](/en/genetics).-->
 我们将在[下一部分](/en/genetics)中实现对前端的登录。
...
<!-- In the next exercises, basics of user management will be implemented for the Bloglist application. The safest way is to follow the story from gastroenterology chapter [User administration](/en/gastroenterology/user_administration) to the chapter [Token-based authentication](/en/gastroenterology/token_authentication). You can of course also use your creativity.-->
 在接下来的练习中，将为Bloglist应用实现用户管理的基础知识。最安全的方法是从第四章节的[用户管理](/en/gastroenterology/user_administration)到[基于令牌的认证](/en/gastroenterology/token_authentication)这一章的故事。当然，你也可以发挥你的创造力。
...
<!-- Do not save passwords to the database as clear text, but use the <i>bcrypt</i> library like we did in gastroenterology chapter [Creating new users](/en/gastroenterology/user_administration#creating-users).-->
 不要将密码以明文形式保存到数据库中，而是使用<i>bcrypt</i>库，就像我们在第四章节[创建新用户](/en/gastroenterology/user_administration#creating-users)中所做的那样。
...
<!-- **NB** Do not test password restrictions with Mongoose validations. It is not a good idea because the password received by the backend and the password hash saved to the database are not the same thing. The password length should be validated in the controller like we did in [ENT](/en/ENT/node_js_and_express) before using Mongoose validation.-->
 **NB** 不要用Mongoose验证来测试密码限制。这不是一个好主意，因为后端收到的密码和保存在数据库中的密码哈希值不是一回事。在使用Mongoose验证之前，应该像我们在[第3章节](/en/ENT/node_js_and_express)中那样在控制器中验证密码长度。
...
<!-- Modify adding new blogs so that when a new blog is created,  <i>any</i> user from the database is designated as its creator (for example the one found first). Implement this according to gastroenterology chapter [populate](/en/gastroenterology/user_administration#populate).-->
 修改添加新的博客，以便当一个新的博客被创建时，数据库中的任何<i></i>用户都被指定为它的创建者（例如，首先找到的那个）。根据第4章节章节[populate](/en/gastroenterology/user_administration#populate)来实现。
...
<!-- Implement token-based authentication according to gastroenterology chapter [Token authentication](/en/gastroenterology/token_authentication).-->
 根据第四章节[令牌认证](/en/gastroenterology/token_authentication)实施基于令牌的认证。
...
<!-- [This example](/en/gastroenterology/token_authentication) from gastroenterology shows taking the token from the header with the _getTokenFrom_ helper function.-->
 [这个例子](/en/gastroenterology/token_authentication)来自第四章节，显示了用_getTokenFrom_辅助函数从头文件中获取令牌。

<!-- If you used the same solution, refactor taking the token to a [middleware](/en/ENT/node_js_and_express#middleware). The middleware should take the token from the <i>Authorization</i> header and place it to the <i>token</i> field of the <i>request</i> object.-->
 如果你使用了相同的解决方案，请将获取令牌重构为一个[中间件](/en/ENT/node_js_and_express#middleware)。中间件应该从<i>Authorization</i>头中获取令牌，并将其放到<i>request</i>对象的<i>token</i>字段中。
...
<!-- Remember that a normal [middleware](/en/ENT/node_js_and_express#middleware) is a function with three parameters, that at the end calls the last parameter <i>next</i> in order to move the control to next middleware:-->
 记住，一个正常的[中间件](/en/ENT/node_js_and_express#middleware)是一个有三个参数的函数，在最后调用最后一个参数<i>next</i>，以便将控制权转移到下一个中间件。
