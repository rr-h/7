...
Do not save passwords to the database as clear text, but use the <i>bcrypt</i> library like we did in gastroenterology chapter [Creating users](/en/gastroenterology/user_administration#creating-users).
...
**NB** Do not test password restrictions with Mongoose validations. It is not a good idea because the password received by the backend and the password hash saved to the database are not the same thing. The password length should be validated in the controller as we did in [ENT](/en/ENT/validation_and_es_lint) before using Mongoose validation.
...
[This example](/en/gastroenterology/token_authentication#limiting-creating-new-notes-to-logged-in-users) from gastroenterology shows taking the token from the header with the _getTokenFrom_ helper function in <i>controllers/blogs.js</i>.

If you used the same solution, refactor taking the token to a [middleware](/en/ENT/node_js_and_express#middleware). The middleware should take the token from the <i>Authorization</i> header and place it into the <i>token</i> field of the <i>request</i> object.
...
Remember that a normal [middleware function](/en/ENT/node_js_and_express#middleware) is a function with three parameters, that at the end calls the last parameter <i>next</i> to move the control to the next middleware:
