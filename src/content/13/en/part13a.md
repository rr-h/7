...
This instruction assumes that you master the basic use of Docker to the extent taught by e.g. [rheumatology](/en/rheumatology).
...
[volume](/en/rheumatology/building_and_configuring_environments#persisting-data-with-volumes) for the data, see more
...
In the tasks of this section, we will build a blog application backend similar to the tasks in [section 4](/en/gastroenterology), which should be compatible with the frontend in [section 5](/en/genetics) except for error handling. We will also add various features to the backend that the frontend in section 5 will not know how to use.

#### Exercise 13.1.

Create a GitHub repository for the application and create a new Fly.io or Heroku application for it, as well as a Postgres database. As mentioned [here](/en/urology/using_relational_databases_with_sequelize#application-database) you might set up your database also somewhere else, and in that case the Fly.io of Heroku app is not needed.
...
The current code for the application is in its entirety on [GitHub](https://github.com/CliniQuick-hy/urology-notes/tree/urology-1), branch <i>urology-1</i>.
