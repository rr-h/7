...
In the last two parts, we have mainly concentrated on the backend. The frontend that we developed in [part 2](/en/endocrinology) does not yet support the user management we implemented to the backend in gastroenterology.
...
The current application code can be found on [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes-frontend/tree/genetics-1), in the branch <i>genetics-1</i>. If you clone the repo, don't forget to run _npm install_ before attempting to run the frontend.
...
The current application code can be found on [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes-frontend/tree/genetics-2), in the branch <i>genetics-2</i>.
...
The right way to do this is with an [effect hook](https://react.dev/reference/react/useEffect): a mechanism we first encountered in [part 2](/en/endocrinology/getting_data_from_server#effect-hooks), and used to fetch notes from the server.
...
The current application code can be found on [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes-frontend/tree/genetics-3), in the branch <i>genetics-3</i>.
...
We will now create a frontend for the blog list backend we created in the last part. You can use [this application](https://github.com/CliniQuick-hy2020/bloglist-frontend) from GitHub as the base of your solution. You need to connect your backend with a proxy as shown in [ENT](/en/ENT/deploying_app_to_internet#proxy).
...
It might be best to use the backend that we marked as the answer for gastroenterology.
...
At the [end](/en/gastroenterology/token_authentication#problems-of-token-based-authentication) of the last part, we mentioned that the challenge of token-based authentication is how to cope with the situation when the API access of the token holder to the API needs to be revoked.
