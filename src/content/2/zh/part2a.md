...
<!-- **NB** As explained in dermatology, when you use the command _console.log_ for debugging, don't concatenate things 'the Java way' with a plus. Instead of writing:-->
...
<!-- If necessary, read more about [debugging React-applications](/en/dermatology/a_more_complex_state_debugging_react_apps#debugging-react-applications).-->
 如果有必要，请阅读更多关于[调试React-应用](/en/dermatology/a_more_complex_state_debugging_react_apps#debugging-react-applications)的内容。
...
<!-- It's worth reading the revision chapter at the end of the previous part [event handlers revisited](/en/dermatology/a_more_complex_state_debugging_react_apps#event-handling-revisited), if it feels like your own knowledge on the topic needs some brushing up.-->
 如果你觉得自己在这个主题上的知识需要精进一下的话，那么值得读一读上一章节[复习事件处理程序](/en/dermatology/a_more_complex_state_debugging_react_apps#event-handling-revisited)最后的修订章节。

<!-- Passing event handlers to the child components of the <i>App</i> component has raised some questions. A small revision on the topic can be found [here](/en/dermatology/a_more_complex_state_debugging_react_apps#passing-event-handlers-to-child-components).-->
 将事件处理程序传递给<i>App</i>组件的子组件，引起了一些问题。关于这个话题的小复习可以在[这里](/en/dermatology/a_more_complex_state_debugging_react_apps#passing-event-handlers-to-child-components)找到。

### Rendering Collections

<!-- We will now do the 'frontend', or the browser-side application logic, in React for an application that's similar to the example application from [cardiology](/en/cardiology)-->
 我们现在将在React中为一个类似于[第0章节](/en/cardiology)中的示例程序做'前端'，或浏览器端应用逻辑。
...
<!-- If you have forgotten what destructuring means and how it works, please review the [section on destructuring](/en/dermatology/component_state_event_handlers#destructuring).-->
 如果你忘记了解构是什么意思以及它是如何工作的，请复习一下一下[解构部分](/en/dermatology/component_state_event_handlers#destructuring) 。
...
<!-- The current code of the application can be found on [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/endocrinology-1).-->
 该应用的当前代码可以在[GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/endocrinology-1)上找到。

<!-- Note that the <i>main</i> branch of the repository contains the code for a later version of the application. The current code is in the branch [endocrinology-1](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/endocrinology-1):-->
 请注意，仓库的<i>main</i>分支包含了该应用的后期版本的代码。目前的代码在分支[endocrinology-1](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/endocrinology-1)中。
...
<!-- Let's finish the code for rendering course contents from exercises 1.1 - 1.5. You can start with the code from the model answers. The model answers for dermatology can be found by going to the [submission system](https://studies.cs.helsinki.fi/stats/courses/fullstackopen), click on <i>my submissions</i> at the top, and in the row corresponding to dermatology under the <i>solutions</i> column click on <i>show</i>. To see the solution to the <i>course info</i> exercise, click on _index.js_ under <i>kurssitiedot</i> ("kurssitiedot" means "course info").-->
