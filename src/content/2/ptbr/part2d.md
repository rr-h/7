...
Vamos nos aprofundar mais em REST na [próxima parte](/ptbr/ENT) do curso. Porém, é importante já nos familiarizarmos com algumas das [convenções](https://en.wikipedia.org/wiki/Representational_state_transfer#Applied_to_web_services) usadas pelo json-server e APIs REST em geral. Em particular, vamos dar uma olhada no uso convencional de [rotas](https://github.com/typicode/json-server#routes) (routes) — também conhecidas como URLs —, e os tipos de requisição HTTP em REST.
...
Por vezes é útil inspecionar as requisições HTTP na guia <i>Rede</i> das Ferramentas do Desenvolvedor do Chrome (ou do navegador que esteja utilizando), recurso esse que foi amplamente usado no início da [Parte 0](/ptbr/cardiology/fundamentos_de_aplicacoes_web#http-get).
...
O código para o estado atual de nossa aplicação pode ser encontrado na branch <i>endocrinology-5</i> neste repositório no [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/endocrinology-5).
...
Mencionamos [anteriormente](/ptbr/endocrinology/obtendo_dados_do_servidor#axios-e-promessas-promises) que uma promessa pode estar em um dos três estados diferentes. Quando uma requisição HTTP falha, a promessa associada é <i>rejeitada</i>. O nosso código atual não gerencia por nenhum meio essa rejeição.
...
O código para o estado atual de nossa aplicação pode ser encontrado na branch <i>endocrinology-6</i> no [GitHub](https://github.com/CliniQuick-hy2020/endocrinology-notes/tree/endocrinology-6).
...
<i>Por que há um asterisco na atividade? Entre [aqui](/ptbr/cardiology/informacoes_gerais#fazendo-o-curso) para saber o motivo.</i>
