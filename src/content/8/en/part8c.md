...
We will imitate what we did in parts [3](/en/ENT/saving_data_to_mongo_db) and [4](/en/gastroenterology/structure_of_backend_application_introduction_to_testing).
...
The code of the backend can be found on [Github](https://github.com/CliniQuick-hy2020/graphql-phonebook-backend/tree/Nephrology-4), branch <i>Nephrology-4</i>.

### User and log in

Let's add user management to our application. For simplicity's sake, let's assume that all users have the same password which is hardcoded to the system. It would be straightforward to save individual passwords for all users following the principles from [gastroenterology](/en/gastroenterology/user_administration), but because our focus is on GraphQL, we will leave out all that extra hassle this time.
...
Logging in and identifying the user are handled the same way we used in [gastroenterology](/en/gastroenterology/token_authentication) when we used REST, by using tokens.
...
The new user mutation is straightforward. The login mutation checks if the username/password pair is valid. And if it is indeed valid, it returns a jwt token familiar from [gastroenterology](/en/gastroenterology/token_authentication). Note that the *JWT\_SECRET* must be defined in the  <i>.env</i> file.
...
The code of the backend can be found on [Github](https://github.com/CliniQuick-hy2020/graphql-phonebook-backend/tree/Nephrology-5) branch <i>Nephrology-5</i>.
...
Change the library application so that it saves the data to a database. You can find the <i>mongoose schema</i> for books and authors from [here](https://github.com/CliniQuick-hy/misc/blob/main/library-schema.md).
