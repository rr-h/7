...
Earlier in this part we [warned](/en/Neurology/first_steps_with_type_script#type-assertion) about the dangers of type assertions, but in our case the assertion is ok since we are sure that the file *index.html* indeed has this particular id and the function is always returning a HTMLElement.
...
In this example, we have a component called *Welcome* to which we pass a *name* as a prop. It then renders the name to the screen. We know that the *name* should be a string, and we use the [prop-types](https://www.npmjs.com/package/prop-types) package introduced in [genetics](/en/genetics/props_children_and_proptypes#prop-types) to receive hints about the desired types of a component's props and warnings about invalid prop types.
...
This exercise is similar to the one you have already done in [dermatology](/en/dermatology/java_script#exercises-1-3-1-5) of the course, but with TypeScript and some extra tweaks. Start off by modifying the contents of *main.tsx* to the following:
...
TypeScript compiler has now no clue what is the type of the parameter, so that is why the type is the infamous implicit any that we want to [avoid](/en/Neurology/first_steps_with_type_script#the-horrors-of-any) at all costs. The React TypeScript cheatsheet comes again to rescue, the chapter about
...
So our code is essentially as safe as it would be if a [type assertion](/en/Neurology/first_steps_with_type_script#type-assertion) would be used:
...
Giving a type parameter to *axios.get* might be ok if we are *absolutely sure* that the backend behaves correctly and returns always the data in the correct form. If we want to build a robust system we should prepare for surprises and parse the response data in the frontend, similarly to what we did [in the previous section](/en/Neurology/typing_an_express_app#proofing-requests) for the requests to the backend.
...
The code of the typed notes can be found [here](https://github.com/CliniQuick-hy2020/typed-notes).
...
Let us now build a frontend for the Ilari's flight diaries that was developed in [the previous section](/en/Neurology/typing_an_express_app). The source code of the backend can be found in [this GitHub repository](https://github.com/CliniQuick-hy2020/flight-diary).
