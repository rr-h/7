...
<!-- In this example, we have a component called <i>Welcome</i> to which we pass a <i>name</i> as a prop. It then renders the name to the screen.  We know that the <i>name</i> should be a string, and we use the [prop-types](https://www.npmjs.com/package/prop-types) package introduced in [genetics](/en/genetics/props_children_and_proptypes#prop-types) to receive hints about the desired types of a component's props and warnings about invalid prop types.-->
 在这个例子中，我们有一个叫做<i>Welcome</i>的组件，我们把一个<i>name</i>作为prop传给它。然后它将这个名字渲染到屏幕上。  我们知道<i>name</i>应该是一个字符串，我们使用[genetics](/en/genetics/props_children_and_proptypes#prop-types)中介绍的[prop-types](https://www.npmjs.com/package/prop-types)包来接收关于组件的prop的理想类型的提示和关于无效prop类型的警告。
...
<!-- This exercise is similar to the one you have already done in [dermatology](/en/dermatology/java_script#exercises-1-3-1-5) of the course, but with TypeScript and some extra tweaks. Start off by modifying the contents of <i>index.tsx</i> to the following:-->
 这个练习与你在课程的[第一章节](/en/dermatology/java_script#exercises-1-3-1-5)中已经完成的练习类似，但使用TypeScript和一些额外的调整。首先，将<i>index.tsx</i>的内容修改为以下内容。
...
<!-- You can also browse the folder structure to get some insight into the application's functionality and/or the architecture used. These are not always clear, and the developers might have chosen a way to organize code that is not familiar to you. The [sample project](https://github.com/CliniQuick-hy2020/patientor) used in the rest of this part is organized, feature-wise. You can see what pages the application has, and some general components, e.g. modals and state. Keep in mind that the features may have-->
你也可以浏览文件夹结构，以了解该应用的功能和/或使用的架构。这些并不总是清晰的，开发者可能选择了一种你不熟悉的方式来组织代码。本章节其余部分使用的[示例项目](https://github.com/CliniQuick-hy2020/patientor)是按功能组织的。你可以看到应用有哪些页面，以及一些一般的组件，例如模态和状态。请记住，这些功能可能有
...
<!-- It's time to get our hands dirty finalizing the frontend for the backend we built in [exercises 9.8.-9.13](/en/Neurology/typing_the_express_app).-->
 是时候动手为我们在[练习9.8.-9.13](/en/Neurology/typing_the_express_app)中建立的后台完成前端了。
...
<!-- In our case, the "global", shared data is the application state <i>and</i> the dispatch function that is used to make changes to data. In many ways our code works much like the Redux-based state management we used in [immunology](/en/immunology), but is more lightweight since it does not require the use of any external libraries. This part assumes that you are at least familiar with the way Redux works, e.g. you should have covered at least [the first section](/en/immunology/flux_architecture_and_redux) of immunology.-->
 在我们的例子中，"全局"、共享的数据是应用状态<i>和</i>调度函数，用于对数据进行更改。在许多方面，我们的代码很像我们在[第6章节](/en/immunology)中使用的基于Redux的状态管理，但由于它不需要使用任何外部库，所以更加轻量级。这一部分假设你至少熟悉Redux的工作方式，例如，你至少应该涵盖第六章节的[第一节](/en/immunology/flux_architecture_and_redux)。
...
<!-- The reducer looks quite similar to the ones we wrote in [immunology](/en/immunology) before we started to use the Redux Toolkit. It changes the state for each type of action:-->
 减速器看起来和我们在开始使用Redux工具包之前在[第6章节](/en/immunology)写的那些很相似。它为每种类型的动作改变状态。
...
<!-- The main difference is  that the state is now a dictionary (or an object), instead of the array that we used in [immunology](/en/immunology).-->
 主要区别在于，现在的状态是一个字典（或一个对象），而不是我们在[第六章节](/en/immunology)中使用的数组。
...
<!-- The Application uses [MaterialUI](https://material-ui.com/) that we covered in [MSK](/en/MSK/more_about_styles) for styling. You may also use it for the new components but that is up to you since our main focus now is TypeScript.-->
 应用使用我们在[第7章节](/en/MSK/more_about_styles)中提到的[MaterialUI](https://material-ui.com/)进行样式设计。你也可以把它用于新的组件，但这取决于你，因为我们现在的主要重点是TypeScript。

<!-- The Application also uses [React Router](https://reactrouter.com/en/main/start/tutorial)  to control which view is visible in the frontend. You might want to have a look at [MSK](/en/MSK/react_router) if you don't yet have a grasp on how the router works.-->
 应用还使用[React Router](https://reactrouter.com/en/main/start/tutorial)来控制哪个视图在前端可见。如果你还没有掌握路由器的工作原理，你可能想看一下[第7章节](/en/MSK/react_router)。
...
<!-- Define [action creator functions](/en/immunology/flux_architecture_and_redux#action-creators) in the file <i>src/state/reducer.ts</i> and refactor the code to use them.-->
 在文件<i>src/state/reducer.ts</i>中定义[动作创建者函数](/en/immunology/flux_architecture_and_redux#action-creators)并重构代码以使用它们。
...
<!-- In the [exercise 9.10.](/en/Neurology/typing_the_express_app#exercises-9-10-9-11) we implemented an endpoint for fetching information about various diagnoses, but we are still not using that endpoint at all.-->
 在[练习9.10.](/en/Neurology/typing_the_express_app#exercises-9-10-9-11)中，我们实现了一个用于获取各种诊断信息的端点，但我们仍然完全没有使用这个端点。
...
<!-- Let's ditch our old patient seed data from the backend and start using [this expanded format](https://github.com/CliniQuick-hy/misc/blob/master/patients.ts).-->
 让我们从后端抛弃旧的病人种子数据，开始使用[这个扩展格式](https://github.com/CliniQuick-hy/misc/blob/master/patients.ts)。
...
<!-- Note that the file [FormField.tsx](https://github.com/CliniQuick-hy2020/patientor/blob/master/src/AddPatientModal/FormField.tsx#L58) has a ready-made component called _DiagnosisSelection_ that can be used for setting the field <i>diagnoses</i>.-->
 注意文件[FormField.tsx](https://github.com/CliniQuick-hy2020/patientor/blob/master/src/AddPatientModal/FormField.tsx#L58)有一个现成的组件叫_DiagnosisSelection_，可以用来设置字段<i>diagnoses</i>。
...
<!-- Note that the "exam done in Moodle" note refers to the [Full Stack Open course's exam](/en/cardiology/general_info#sign-up-for-the-exam), which has to be completed before you can earn credits from this part.-->
 注意 "在Moodle中完成的考试 "是指[全栈开放课程的考试](/en/cardiology/general_info#sign-up-for-the-exam)，在你从这部分获得学分之前必须完成。
