...
The exercises in this seventh part of the course differ a bit from the ones before. In this and the next chapter, as usual, there are [exercises related to the theory in the chapter](/en/MSK/react_router#exercises-7-1-7-3).
...
Following immunology, we return to React without Redux.
...
In an [old school web app](/en/cardiology/fundamentals_of_web_apps#traditional-web-applications), changing the page shown by the application would be accomplished by the browser making an HTTP GET request to the server and rendering the HTML representing the view that was returned.
...
Let's examine a slightly modified version from the previous example. The complete code for the updated example can be found [here](https://github.com/CliniQuick-hy2020/misc/blob/master/router-app-v1.js).
...
The completed code can be found [here](https://github.com/CliniQuick-hy2020/misc/blob/master/router-app-v2.js).
...
Let's return to working with anecdotes. Use the redux-free anecdote app found in the repository <https://github.com/CliniQuick-hy2020/routed-anecdotes> as the starting point for the exercises.
