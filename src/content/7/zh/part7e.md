...
<!-- Let's get to know the main features of Class Components by producing yet another very familiar anecdote application. We store the anecdotes in the file <i>db.json</i> using <i>json-server</i>. The contents of the file are lifted from [here](https://github.com/CliniQuick-hy/misc/blob/master/anecdotes.json).-->
...
<!-- [This repository](https://github.com/CliniQuick-hy2020/create-app) provides one possible starting point for the organization of "single repository code".-->
...
<!-- In [Nephrology](/en/Nephrology), our topic is GraphQL, which provides a nice mechanism for notifying clients when there are changes in the backend data.-->
 在[第8章节](/en/Nephrology)中，我们的主题是GraphQL，它提供了一个很好的机制，当后台数据有变化时，可以通知客户端。

### Virtual DOM

<!-- The concept of the Virtual DOM often comes up when discussing React. What is it all about? As mentioned in [cardiology](/en/cardiology/fundamentals_of_web_apps#document-object-model-or-dom), browsers provide a [DOM API](https://developer.mozilla.org/fi/docs/DOM) through which the JavaScript running in the browser can modify the elements defining the appearance of the page.-->
...
<!-- However, MVC architecture is not usually mentioned when talking about React applications. Furthermore, if we are using Redux, then the applications follow the [Flux](https://facebook.github.io/flux/docs/in-depth-overview) architecture and the role of React is even more focused on creating the views. The business logic of the application is handled using the Redux state and action creators. If we're using [redux thunk](/en/immunology/communicating_with_server_in_a_redux_application#asynchronous-actions-and-redux-thunk) familiar from immunology, then the business logic can be almost completely separated from the React code.-->
...
<!-- One year old project that is used by the [Neurology](/en/Neurology) of his course already have quite a few outdated dependencies:-->
...
<!-- Sometimes, the [dynamic typing](https://developer.mozilla.org/en-US/docs/Glossary/Dynamic_typing) of JavaScript variables creates annoying bugs. In genetics, we talked briefly about [PropTypes](/en/genetics/props_children_and_proptypes#prop-types): a mechanism which enables one to enforce type checking for props passed to React components.-->
...
<!-- [Redux-saga](https://redux-saga.js.org/) provides an alternative way to make asynchronous actions for [redux thunk](/en/immunology/communicating_with_server_in_a_redux_application#asynchronous-actions-and-redux-thunk) familiar from immunology. Some embrace the hype and like it. I don't.-->
...
<!-- You can take advantage of your React know-how when developing mobile applications using Facebook's extremely popular [React Native](https://facebook.github.io/react-native/) library, which is the topic of [OBGyn](/en/OBGyn) of the course.-->
